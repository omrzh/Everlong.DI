...
internal struct HashCode
{
  ...
  { v1 = seed + Prime1 + Prime2; v2 = seed + Prime2; v3 = seed; v4 = seed - Prime1; }
  ...
  private void Add(int value)
  {
    ...
    if (position == 0) this.queue1 = val;
    else if (position == 1) this.queue2 = val;
    else if (position == 2) this.queue3 = val;
    else
    {
      if (previousLength == 3) Initialize(out this.v1, out this.v2, out this.v3, out this.v4);
  ...
  public readonly int ToHashCode()
  {
    ...
    if (position > 0)
    {
      ...
      if (position > 1) hash = QueueRound(hash, this.queue2);
      if (position > 2) hash = QueueRound(hash, this.queue3);
    }
    hash = MixFinal(hash);
    return (int)hash;
  }
